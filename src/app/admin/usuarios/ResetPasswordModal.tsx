...
export function ResetPasswordModal({ userId, username }: Props) {
  ...
  return (
    <>
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 transition hover:bg-amber-100 dark:border-amber-800/60 dark:bg-amber-950/30 dark:text-amber-400 dark:hover:bg-amber-900/40"
      >
            ...
            <div className="px-5 py-5">
              {done ? (
                <div className="flex flex-col items-center gap-3 py-4 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">El usuario deberá iniciar sesión con la nueva clave.</p>
                </div>
              ) : (
                <>
                  {/* Warning */}
                  <div className="mb-4 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50/60 px-3 py-2.5 dark:border-amber-800/50 dark:bg-amber-950/20">
                    ...
                    <p className="text-xs text-amber-700 dark:text-amber-400">
                      Esta acción cambia la contraseña inmediatamente. Compartí la nueva clave con el usuario de forma segura.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        required
                        className={[
                          'h-10 w-full rounded-lg border bg-background px-3 text-sm focus:outline-none focus:ring-2',
                          mismatch
                    ...
                    {password.length > 0 && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-muted-foreground">
                          {password.length < 3 ? 'Muy corta' : password.length < 6 ? 'Corta' : password.length < 9 ? 'Media' : 'Fuerte'}
                        </span>
                      </div>
                    )}
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      <button
                        ...
                        className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-amber-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-amber-600 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        {loading
