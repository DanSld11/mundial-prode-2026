...
export default function AdminPage() {
  ...
  async function seedTeams() {
    ...
    if (!token) { toast.error('No autenticado'); setLoadingTeams(false); return }
  ...
  async function seedMatches() {
    ...
    const token = getToken()
    if (!token) { toast.error('No autenticado'); setLoadingMatches(false); return }
    ...
    const venues = [
      ...
      { venue: 'Azteca Stadium', city: 'Mexico City, MX' },
      { venue: 'Akron Stadium', city: 'Guadalajara, MX' },
      { venue: 'BMO Field', city: 'Toronto, CA' },
      { venue: 'BC Place', city: 'Vancouver, CA' },
    ]
    ...
    groups.forEach((group, gi) => {
      const gt = SEED_TEAMS.filter(t => t.group_name === group)
      const [t1, t2, t3, t4] = gt
      const pairs = [[t1, t2], [t3, t4], [t1, t3], [t2, t4], [t1, t4], [t2, t3]]
      pairs.forEach(([h, a], idx) => {
        const dayOffset = Math.floor((gi * 6 + idx) / 4)
        const hourOffset = (idx % 4) * 3
        const d = new Date(baseDate.getTime() + dayOffset * 86400000 + hourOffset * 3600000)
        const v = venues[(matchNumber - 1) % venues.length]
        matches.push({
          match_number: matchNumber++,
          stage: 'group',
          group_name: group,
          home_team_id: teamMap.get(h.code),
          away_team_id: teamMap.get(a.code),
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="flex gap-4">
          <Button onClick={seedTeams} disabled={loadingTeams} variant="outline">
            {loadingTeams ? 'Insertando...' : 'Insertar 48 Equipos'}
          </Button>
          <Button onClick={seedMatches} disabled={loadingMatches} variant="outline">
