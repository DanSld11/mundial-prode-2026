...
export async function createPlayerAction(input: {
  teamId: string; name: string; shirtNumber: string; position: string
}) {
...
export async function togglePlayerActiveAction(playerId: string, active: boolean) {
  if (!await assertAdmin()) return { error: 'Sin permisos' }
  const db = createServiceRoleClient()
  const { error } = await db.from('players').update({ active: !active, updated_at: new Date().toISOString() }).eq('id', playerId)
  if (error) return { error: error.message }
  revalidatePath('/admin/jugadores')
  return { success: true }
}
...
export async function updatePlayerAction(playerId: string, updates: Record<string, unknown>) {
  if (!await assertAdmin()) return { error: 'Sin permisos' }
  const db = createServiceRoleClient()
  const { error } = await db.from('players').update({ ...updates, updated_at: new Date().toISOString() }).eq('id', playerId)
  if (error) return { error: error.message }
  revalidatePath('/admin/jugadores')
  return { success: true }
}
