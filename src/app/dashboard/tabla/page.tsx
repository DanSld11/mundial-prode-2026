...
import { Table2, Trophy, Medal, Award } from 'lucide-react'
      ...
      {entries.length === 0 && (
        <div className="rounded-xl border bg-card p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            El leaderboard se actualizará cuando los usuarios empiecen a jugar.
          </p>
        </div>
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="px-4 py-3 text-center font-medium hidden md:table-cell">Total</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center hidden md:table-cell">{entry.predictions_total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
