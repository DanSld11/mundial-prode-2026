...
export default async function GruposPage() {
  ...
  const teamsList = (teams ?? []) as unknown as Team[]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {groups.map((group) => {
          const groupTeams = teamsList.filter((t) => t.group_name === group)
          return (
            <Card key={group}>
              ...
              <CardContent className="p-0">
                <div className="divide-y">
                  {groupTeams.map((team) => (
                    <div key={team.id} className="flex items-center justify-between px-4 py-3 hover:bg-muted/30">
                      <div className="flex items-center gap-3">
                        <span className="text-xl">{team.flag_emoji}</span>
                        <span className="font-medium text-sm">{team.name_es}</span>
                      </div>
                      <span className="text-xs text-muted-foreground uppercase tracking-wider">{team.code}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
      ...
      {teamsList.length === 0 && (
        <div className="rounded-xl border bg-card p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">El administrador debe cargar los equipos desde el panel de admin.</p>
        </div>
      )}
    </div>
  )
